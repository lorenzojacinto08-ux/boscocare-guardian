...
import type { GuidanceScheduleHistory } from "@/types/tables";

const GuidanceScheduleHistory = () => {
  const navigate = useNavigate();
  const [records, setRecords] = useState<GuidanceScheduleHistory[]>([]);
  const [loading, setLoading] = useState(true);
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editingRecord, setEditingRecord] =
    useState<GuidanceScheduleHistory | null>(null);
  ...
  const handleEdit = (record: GuidanceScheduleHistory) => {
            ...
            <CardHeader className="flex flex-row items-center justify-between">
              ...
              <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
                ...
                <DialogContent className="max-w-2xl">
                  <DialogHeader>
                    <DialogTitle>
                      {editingRecord ? "Edit" : "Add"} Schedule History
                    </DialogTitle>
                    <DialogDescription>
                      {editingRecord ? "Update" : "Create a new"} completed
                      activity record
                    </DialogDescription>
                  </DialogHeader>
